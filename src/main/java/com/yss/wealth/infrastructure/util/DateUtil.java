...
public class DateUtil {
    ...
    /**
     * @description 判断当前时间是否在传入时间之后。比如传入“15:00:00”。即判断当前时间是不是过了下午三点。
     * @author xuanfei.Chen
     * @date 2020/6/4 11:03
     */
    public static boolean isAfterTime(String time) {
        ...
        return curTime.after(scaTime);
    }

    /**
     * @Description: 将util.Date转换为sql.Date
     * @Param:
     * @return:
     * @Author: yinyuyi
     * @Date: 2020/10/29 16:28
     */
    public static java.sql.Date convertUitlDate(Date date) {
        if (null == date) {
            return null;
        }
    ...
    public static int daysBetween(String sDate, String eDate) {
        ...
        Calendar calendarStart = Calendar.getInstance();// 获取日历对象
        ...
        try {
            dateStart = simpleDateFormat.parse(sDate);//字符串转Date
            dateEnd = simpleDateFormat.parse(eDate);
            calendarStart.setTime(dateStart);// 设置日历
        ...
        return (int) ((timeEnd - timeStart) / (1000 * 3600 * 24));//计算相差天数
